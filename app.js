  ...
  try {
    ...
    if (isEditing) {
      ...
      batch.set(invoiceRef, {
        ...
        updatedAt: now,
        // Clear the editing lock as part of the save
...
async function startEditInvoice(invoiceNumber) {
  ...
  // FIX #2: Check if someone else is already editing this invoice
  if (invoice.editingBy && invoice.editingBy !== getCurrentUser()) {
    // Stale lock check: ignore locks older than 30 minutes
    const lockTs = invoice.editingSince && invoice.editingSince.toDate ? invoice.editingSince.toDate() : null;
    const stale = lockTs && (Date.now() - lockTs.getTime()) > 30 * 60 * 1000;
    if (!stale) {
  ...
  try {
    ...
    if (snapshot) {
      ...
      for (const [partId, deduct] of deductByPart.entries()) {
        ...
        if (part) {
          batch.update(db.collection("parts").doc(partId), {
            currentQuantity: Number(part.currentQuantity || 0) - deduct
          });
        }
      }
    }
