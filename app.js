  ...
  try {
    const result = await db.runTransaction(async tx => {
      ...
      for (const [partId, diffQty] of diffByPart.entries()) {
        ...
        if (!partDoc.exists) throw new Error(`Part ${partId} was not found.`);
        const partData = partDoc.data();
        const beforeQty = Number(partData.currentQuantity || 0);
        const afterQty = beforeQty - diffQty;
        if (afterQty < 0) {
          throw new Error(`Not Enough Inventory for ${partData.name}. Available: ${beforeQty}, Requested: ${diffQty}`);
        }
        tx.update(partRef, { currentQuantity: afterQty, updatedAt: now });
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      for (const [partId, addBack] of restoredByPart.entries()) {
        ...
        const partData = partDoc.data();
        const beforeQty = Number(partData.currentQuantity || 0);
        const afterQty = beforeQty + Number(addBack || 0);
        tx.update(partRef, { currentQuantity: afterQty, updatedAt: now });
        tx.set(makeMovementRef(), {
          timestamp: now,
          ...
          partName: partData.name || "",
          rackingType: partData.rackingType || "",
          quantityChange: Number(addBack || 0),
          beforeQuantity: beforeQty,
          afterQuantity: afterQty,
          user,
...
function buildAndDownloadInvoicePdf(invoice) {
  ...
  doc.setFontSize(18);
  doc.text("Mc Racking Invoice", 14, 20);

  doc.setFontSize(10);
  doc.text("Mc Racking", 150, 16);
  doc.text("Racking Inventory & Usage", 150, 22);
  doc.text("Phone: __________________", 150, 28);
  doc.text("Email: __________________", 150, 34);
  ...
  if (invoice.workOrderNumber) { doc.text(`Work Order #: ${invoice.workOrderNumber}`, 14, y); y += 8; }
  if (invoice.poNumber) { doc.text(`PO #: ${invoice.poNumber}`, 14, y); y += 8; }
  ...
  if (invoice.notes) {
    ...
    finalY += 6;
    const wrapped = doc.splitTextToSize(String(invoice.notes), 180);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try { await ensureSignedIn(); } catch (err) {
    console.error("Firebase sign-in failed:", err);
    setConnectionStatus("Auth error", "error");
    showMessage("Could not sign in to Firebase. Enable Anonymous Authentication in Firebase.", true);
    return;
  }
