...
function attachListeners() {
  ...
  db.collection("settings").doc("trucks").onSnapshot(doc => {
    if (doc.exists && Array.isArray(doc.data().list)) {
      state.trucks = doc.data().list.slice().filter(Boolean).sort((a, b) => a.localeCompare(b));
    } else {
      state.trucks = [];
    }
...
function renderSelects() {
  ...
  ["checkoutTruck", "invoiceTruck"].forEach(id => {
    ...
    select.innerHTML = state.trucks.map(t => `<option value="${escapeHtml(t)}">${escapeHtml(t)}</option>`).join("");
...
function renderTruckInventoryTable() {
  const body = qs("truckInventoryBody");
  if (!body) return;
  const visible = state.truckInventory.filter(t => Number(t.quantity || 0) > 0);
  if (!visible.length) {
    body.innerHTML = `<tr><td colspan="6" class="muted" style="text-align:center;padding:20px;">No inventory is currently loaded on trucks.</td></tr>`;
    return;
  }
