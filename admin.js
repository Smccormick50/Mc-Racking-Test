...
async function startAdmin() {
  ...
  try { await ensureSignedIn(); console.log("Admin test UID:", auth.currentUser && auth.currentUser.uid); } catch (err) {
    console.error("Firebase sign-in failed:", err);
    setConnectionStatus("Auth error", "error");
    showAdminMessage("Could not sign in to Firebase. Enable Anonymous Authentication in Firebase.", true);
    return;
  }
  setupTabs();
