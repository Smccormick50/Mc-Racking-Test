...
function attachAdminListeners() {
  ...
  db.collection("settings").doc("trucks").onSnapshot(doc => {
    adminState.trucks = (doc.exists && Array.isArray(doc.data().list))
      ? doc.data().list.slice().filter(Boolean).sort((a, b) => a.localeCompare(b))
      : [];
    renderTrucksList();
  });
...
function renderTrucksList() {
  ...
  if (!adminState.trucks.length) {
    ul.innerHTML = `<li class="muted">No trucks yet. Add one above.</li>`;
    return;
  }
  const sorted = adminState.trucks.slice().sort((a, b) => a.localeCompare(b));
  ul.innerHTML = sorted.map(t => `
    <li>
      <span>${escapeHtml(t)}</span>
      <button type="button" class="danger" data-truck="${escapeHtml(t)}">Remove</button>
    </li>
  `).join("");
  ul.querySelectorAll("button[data-truck]").forEach(btn => {
    btn.addEventListener("click", () => removeTruck(btn.dataset.truck));
  });
}
...
async function removeTruck(name) {
  ...
  const newList = adminState.trucks.filter(t => t !== name);
  try {
    await db.collection("settings").doc("trucks").set({ list: newList });
